...
class KazooRetry(object):
    ...
    def __init__(self, max_tries=1, delay=0.1, backoff=2, max_jitter=0.8,
                 ...
                 deadline=None, interrupt=None, logger=None):
        ...
        while True:
            try:
                ...
                self.logger.debug('Retry: got a %s(%s) (attempt: %s/%s, time: %s left)', type(e).__name__, e,
                        self._attempts+1,
                        ('unlimited' if self.max_tries is None else (self.max_tries+1)),
                        ('forever' if self._cur_stoptime is None else (self._cur_stoptime - now)))
